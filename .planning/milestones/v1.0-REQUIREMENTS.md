# Requirements Archive: v1.0 D2P CLI
...
- [x] **CMD-01**: User can run `d2p ts reactive uniswap-v3` to deploy UniswapV3Reactive
- [x] **CMD-02**: CLI accepts `--rpc-url` flag with `ETH_RPC_URL` env var fallback
- [x] **CMD-03**: CLI accepts `--private-key` flag with `ETH_PRIVATE_KEY` env var fallback
- [x] **CMD-04**: CLI accepts `--callback` flag for callback proxy address (required, no default)
- [x] **CMD-05**: CLI accepts `--value` flag with human-friendly unit parsing (e.g., `10react`, `0.01ether`), defaulting to `10 react`
- [x] **CMD-06**: `--legacy` is baked into the `forge create` path (not user-supplied)
- [x] **CMD-07**: `d2p ts reactive --help` shows usage with examples
- [x] **CMD-08**: `d2p --version` shows version from Cargo.toml
- [x] **CMD-09**: CLI accepts `--project` flag for Solidity project root path, defaulting to CWD; forge create runs from this directory
...
- [x] **OUT-02**: On failure, stderr shows which command was attempted and what went wrong
- [x] **OUT-03**: Exit code 0 on verified success, exit code 1 on any failure
